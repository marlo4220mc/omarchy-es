...
Item {
  ...
  readonly property string promptText: root.step === "message" ? "Mensaje del recordatorio" : "Recordar en minutos"
  ...
  function submit() {
    ...
    if (root.step === "minutes") {
      ...
      if (!nextMinutes) {
        Quickshell.execDetached([root.omarchyPath + "/bin/omarchy-notification-send", "Recordatorio no válido", "Introduce el número de minutos"])
        return
      }
