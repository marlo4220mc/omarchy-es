...
Item {
  ...
  Process {
    ...
    onExited: function(exitCode) {
      if (exitCode !== 0) {
        if (root.syncConfigured()) root.syncStatusText = "Error al crear la carpeta de sincronización"
        root.finishSyncRun()
        return
      }
  ...
  Process {
    ...
    onRunningChanged: root.updateSyncRunning()
    onExited: function(exitCode) {
      if (exitCode !== 0 && root.syncConfigured()) root.syncStatusText = "Error al escanear la sincronización"
      root.finishSyncRun()
    }
