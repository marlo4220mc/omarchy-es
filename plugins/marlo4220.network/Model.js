...
function bandLabel(band) {
  if (band === "auto") return "Automático"
  if (!band) return ""
  return band + "ghz"
}
...
function bandSectionTitle(selected, current) {
  if (selected !== "auto") return "BANDA WI-FI"

  var label = bandLabel(current)
  if (label === "") return "BANDA WI-FI"

  return "BANDA WI-FI: " + label.toUpperCase()
}

function bandTooltip(band) {
  if (band === "auto") return "Deja que Wi-Fi elija la banda"
  if (!band) return ""
  return "Permanecer en " + bandLabel(band)
}
...
function formatPingLatency(ms, hasSamples) {
  ...
  if (!isFinite(value) || value < 0) return "Tiempo agotado"
...
function wifiSectionTitle(wifiNetworks, index) {
  ...
  if (net.known && index === 0) return "REDES CONOCIDAS"
  if (!net.known && (index === 0 || (networks[index - 1] && networks[index - 1].known))) return "OTRAS REDES"
  return ""
}
...
function networkFailureReason(reason, needsCredentials, reasons) {
  var r = reasons || {}
  if (needsCredentials && reason === r.NoSecrets) return "Se requiere contraseña"
  if (needsCredentials && reason === r.WifiAuthTimeout) return "Contraseña incorrecta"
  if (reason === r.WifiNetworkLost) return "Red perdida"
  if (reason === r.WifiClientDisconnected) return "Desconectado"
  if (reason === r.WifiClientFailed) return "Fallo de conexión"
