...
Panel {
  ...
  readonly property var activePhrases: [
    "Desenredando cables",
    "Vikingos en streaming",
    "Misterios de emparejamiento",
    "Guiando auriculares",
    "Domando radios",
    "Invocando altavoces",
    "Gestionando códecs",
  ...
  readonly property string heroStatusText: {
    if (!adapter) return "Sin adaptador"
    if (!adapter.enabled) return "Apagado"
  ...
  readonly property string toggleHint: root.adapter && root.adapter.enabled ? "Apagar Bluetooth" : "Encender Bluetooth"
  ...
  function scrollSectionTitle(index) {
    ...
    return rows[index].section === "known" ? "EMPAREJADOS" : "DISPONIBLES"
  }
...
Text {
            textFormat: Text.PlainText
            visible: root.connectedDevices.length === 0 && root.scrollRows.length === 0
            text: !root.adapter ? "Sin adaptador Bluetooth"
                : !root.adapter.enabled ? "Activa Bluetooth para escanear"
  ...
  component DeviceRow: CursorSurface {
    ...
    readonly property string actionTooltip: {
      ...
      if (isDiscovered) return "Emparejar"
      return "Conectar"
    }
    ...
    readonly property string statusText: {
      if (!dev) return ""
      if (action === "forgetting") return "Olvidando…"
