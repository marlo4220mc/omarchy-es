...
function outputVolumeName(volume, muted) {
  if (muted) return "Silenciado"
  var p = Math.round(volume * 100)
  if (p === 0) return "En silencio"
  if (p >= 100) return "Sala de conciertos"
  if (p >= 85) return "Modo fiesta"
  if (p >= 70) return "A tope"
  if (p >= 50) return "Ritmo constante"
  if (p >= 30) return "Escucha relajada"
  if (p >= 15) return "Murmullo"
  return "Susurro"
}
...
function friendlyDeviceLabel(text) {
  ...
  label = label.replace(/\bMicrophones\b/gi, "Micrófono")
  return label
}
...
function nodeLabel(node) {
  ...
  return friendlyDeviceLabel(node.description || p["node.description"] || node.name || "Desconocido")
}
...
function streamLabel(node, players, streams) {
  if (!node) return "Transmisión de audio"
